...
void CCSBuyMenu::ShowPanel( bool bShow )
{
	...
	if ( bShow )
	{
		...
		m_pPlayerModel->ResetRotation(); 
		ResetWeapon();
		m_pItemNameLabel->SetVisible( false );
		m_pItemDescriptionLabel->SetVisible( false );

		vgui::ImagePanel* pTeamLogo = dynamic_cast<vgui::ImagePanel*>(FindChildByName("TeamLogo"));
		if ( pTeamLogo )
		{
			...
			if ( iTeam == 3 ) // TEAM_CT
			{
				...
				pTeamLogo->SetImage("hud/ct_icon");
			}
			else if ( iTeam == 2 ) // TEAM_TERRORIST
			{
				pTeamLogo->SetVisible(true);
				pTeamLogo->SetImage("hud/tr_icon");
			}
			else
			{
		...
		{
			char szName[32];
			Q_snprintf( szName, sizeof(szName), "BuyCategory%d", i );
			vgui::Panel *pCategory = FindChildByName( szName );
			if ( pCategory ) pCategory->SetVisible( true );
			
			Q_snprintf( szName, sizeof(szName), "BuyItem%d", i );
			vgui::Panel *pItem = FindChildByName( szName );
			if ( pItem ) pItem->SetVisible( false );
		}
		vgui::Panel *pItemsBg = FindChildByName( "BuyItemsBackground" );
		if ( pItemsBg ) pItemsBg->SetVisible( false );
		// ==============================================================================
	...
	else
	{
		...
		if ( kvSubMenu )
		{
			for ( int i = 1; i <= 6; i++ )
			{
				...
				Q_snprintf( szName, sizeof(szName), "BuyCategory%d", i );
				vgui::Panel *pCategory = FindChildByName( szName );
				if ( pCategory ) pCategory->SetVisible( false );
			}
			vgui::Panel *pItemsBg = FindChildByName( "BuyItemsBackground" );
			if ( pItemsBg ) pItemsBg->SetVisible( true );
...
void CCSBuyMenu::OnThink()
{
    ...
    if (iLastSeconds == iSecondsLeft)
        return;

    iLastSeconds = iSecondsLeft;
    ...
    V_snprintf(
        sz,
        sizeof(sz),
        "Buy time remaining: %d:%02d",
        iSecondsLeft / 60,
        iSecondsLeft % 60
    );

    if (m_pBuyTimeLeftLabel)
    {
        m_pBuyTimeLeftLabel->SetText(sz);
    }
}