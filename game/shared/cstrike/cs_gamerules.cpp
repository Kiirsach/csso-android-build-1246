...
// 1. CheckMusicDuration 彻底弃用，删除所有 StopSound
void CheckMusicDuration()
{
    // 留空，不再需要主动停止
}

// 2. 修改 PlayMusicSelection
void PlayMusicSelection( IRecipientFilter& filter, CsMusicType_t nMusicType, int nPlayerEntIndex /* = 0 */ )
{
    ...
    if (cl_music_enable.IsValid() && cl_music_enable.GetInt() == 0) return;

    // [状态过滤] 略... (保持原样)

    // ========================================================================
    // 【核心逻辑】不使用 StopSound，但要防止重叠
    // ========================================================================
    if (g_currentPlayingMusic != CSMUSIC_NONE && g_currentPlayingMusic != nMusicType)
    {
        ...
        if (cs_PR)
        {
            ...
            // --- 巧妙处理：不停止声音，而是发送一个 0 音量的同名声音覆盖它 ---
            // 注意：这里需要引擎支持相同 Soundname 的覆盖机制（起源引擎默认 EmitSound 同名脚本会更新参数）
            EmitSound_t es;
            es.m_pSoundName = prevMusicSelection;
            es.m_flVolume = 0.0f; // 关键：音量设为 0
            es.m_nFlags = SND_CHANGE_VOL; // 关键：告诉引擎这只是改变音量
            C_BaseEntity::EmitSound(filter, -1, es);
        }
    }
    ...
    g_bShouldCheckDuration = false; // 既然不需要 StopSound，也就不需要 Check 了
}
